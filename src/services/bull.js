...
CLOUDINARY_QUEUE.renameFile.process(async (job, done) => {
  const {
    data: {
      currentPath, newPath, fileId, postId, resourceType,
    },
  } = job;
  try {
...
CLOUDINARY_QUEUE.renameFile.on('completed', (job, result) => {
  if (!result) console.log('****__QUEUE_JOB__**** Cloudinary renamed file FAILED');

  if (result) console.log('****__QUEUE_JOB__**** Cloudinary renamed file SUCCESS');
});

CLOUDINARY_QUEUE.deleteAsset.on('completed', (job, result) => {
  if (result.result === 'not found') console.log('****__QUEUE_JOB__**** Cloudinary deleted asset FAILED');

  if (result.result === 'ok') console.log('****__QUEUE_JOB__**** Cloudinary deleted asset SUCCESS');
});

FILE_REFERENCE_QUEUE.deleteFile.on('completed', (job, result) => {
  if (!result) console.log('****__QUEUE_JOB__**** FileReference deleted file in DATABASE FAILED');

  if (result) console.log('****__QUEUE_JOB__**** FileReference deleted file in DATABASE SUCCESS');
});
